...
def timetree(tree=None, aln=None, ref=None, seq_meta=None, keeproot=False,
    ...
    #    pi=np.array([0.15, 0.3, 0.3, 0.15, 0.1])
    #    W = np.ones((5,5), dtype=float)
        #W[0,2] = W[2,0] = W[1,3] = W[3,1] = 3.0
    #    W[0,2] = W[2,0] = 2.22
    #    W[1,3] = W[3,1] = 2.14
    #    W[0,1] = W[1,0] = 0.76
    #    W[0,3] = W[3,0] = 0.35
    ...
    #    mtb_gtr = GTR.custom(mu=1.0, pi=pi, W=W, alphabet="nuc")
    #    print(mtb_gtr)
    ...
    if ref != None: #if VCF, fix pi
        ...
        #for lee ?
        #pi = np.array([0.1702, 0.3254, 0.3242, 0.1702, 0.01])
    ...
    #for walker 2018
    tt.run(infer_gtr=infer_gtr, root=reroot, Tc=Tc, time_marginal=marginal, fixed_clock_rate=5e-7, relaxed_clock={"slack":None, "coupling":None}, #fixed_clock_rate=0.000001,
       resolve_polytomies=resolve_polytomies, max_iter=max_iter, fixed_pi=pi, **kwarks)
